...
def profile(model, input_size, custom_ops={}):
    ...
    return int(total_ops.item()), int(total_params.item())
...
def main():
    ...
    ref_bits = 16
    sota_bits = 1
    ref_ops, ref_params = profile(ref_model, (1, 3, 32, 32))
    sota_ops, sota_params = profile(sota_model, (1, 3, 32, 32))
    print(
    ...
    print(
        ...
        f"sota_bits: {sota_bits} \n",
    )

    quantization_rate = sota_bits / ref_bits
    score_ops = sota_ops / ref_ops
    score_params = sota_params / ref_params
    score = (score_ops + score_params) * quantization_rate
    print(
